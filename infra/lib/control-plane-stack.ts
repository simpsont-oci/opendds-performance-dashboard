...
export class ControlPlaneStack extends cdk.Stack {
  constructor(scope: Construct, id: string, props: ControlPlaneStackProps) {
    ...
    const bundleVersion = 'al2023-xerces-3.2.5-v5';
    ...
    const infraProject = new codebuild.Project(this, 'ManageRunStack', {
      ...
      buildSpec: codebuild.BuildSpec.fromObject({version: '0.2', phases: {build: {commands: [
        ...
        'npx cdk "$CDK_ACTION" "OpenDdsPerformanceRun-*" --require-approval never --force -c stage="$STAGE" -c runId="$RUN_ID" -c suite="$SUITE" -c commitSha="$OPENDDS_COMMIT" -c automationCommit="$AUTOMATION_COMMIT" -c configCommit="$CONFIG_COMMIT" -c environmentName="$ENVIRONMENT_NAME" -c instanceType="$INSTANCE_TYPE" -c amiId="$AMI_ID" -c availabilityZone="$AVAILABILITY_ZONE" -c dynamicMulticastRegistration="$DYNAMIC_MULTICAST_REGISTRATION" -c artifactKey="builds/$BUNDLE_VERSION/$OPENDDS_COMMIT/bench.tar.gz" -c configKey="configs/$CONFIG_COMMIT/config.tar.gz"',
      ]}}}),
      environmentVariables: {
        STAGE: {value: props.stage},
        BUNDLE_VERSION: {value: bundleVersion},
        ARTIFACT_BUCKET: {value: artifactBucket.bucketName},
        NIGHTLY_REPO_URL: {value: props.config.nightlyRepoUrl},
      },
    });

    const invoke = (name: string, action: string) => new tasks.LambdaInvoke(this, name, {
      lambdaFunction: coordinator,
      payload: sfn.TaskInput.fromObject({action, 'commitSha.$': '$.commitSha', 'automationCommit.$': '$.automationCommit', 'configCommit.$': '$.configCommit', 'environmentName.$': '$.environmentName', 'suite.$': '$.suite', 'instanceType.$': '$.instanceType', 'amiId.$': '$.amiId', 'availabilityZone.$': '$.availabilityZone', 'topology.$': '$.topology', 'estimatedCostUsd.$': '$.estimatedCostUsd', 'manualOverride.$': '$.manualOverride', 'dynamicMulticastRegistration.$': '$.dynamicMulticastRegistration', 'repeatNonce.$': '$.repeatNonce', 'runId.$': '$.runId'}),
      payloadResponseOnly: true,
    });
    const acquire = new tasks.LambdaInvoke(this, 'Acquire lease and budget', {lambdaFunction: coordinator, payload: sfn.TaskInput.fromObject({action: 'acquire', 'commitSha.$': '$.commitSha', 'automationCommit.$': '$.automationCommit', 'configCommit.$': '$.configCommit', 'environmentName.$': '$.environmentName', 'suite.$': '$.suite', 'instanceType.$': '$.instanceType', 'amiId.$': '$.amiId', 'availabilityZone.$': '$.availabilityZone', 'topology.$': '$.topology', 'estimatedCostUsd.$': '$.estimatedCostUsd', 'manualOverride.$': '$.manualOverride', 'dynamicMulticastRegistration.$': '$.dynamicMulticastRegistration', 'repeatNonce.$': '$.repeatNonce'}), payloadResponseOnly: true});
