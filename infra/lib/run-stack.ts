...
WORKER_WRAPPER`,
    ...
    controllerUserData.addCommands(
      `export RUN_ID='${props.config.runId}' SUITE='${props.config.suite}' OPENDDS_COMMIT='${props.config.commitSha}' AUTOMATION_COMMIT='${props.config.automationCommit}' CONFIG_COMMIT='${props.config.configCommit}' PERFORMANCE_ENVIRONMENT_NAME='${props.config.environmentName}' MULTICAST_REGISTRATION_MODE='${props.config.dynamicMulticastRegistration ? 'dynamic' : 'static'}'`,
